...
public class AddRosterItemDialog extends AlertDialog implements DialogInterface.OnClickListener, TextWatcher {
	...
	public void onClick(DialogInterface dialog, int which) {
		xChatService.addRosterItem(userInputField.getText().toString(), aliasInputField.getText().toString(), mGroupNameView.getGroupName());
	}

	public void afterTextChanged(Editable s) {
		if(MyUtil.validateAccount(s.toString())){
			okButton.setEnabled(true);
			userInputField.setTextColor(MyUtil.getEditTextColor(mMainActivity));
		}else {
